...
import { reseaux } from '../utils/datas';

const ChoixNumeroDestinataireInter = ({navigation}:any) => {
    ...
    const [listeReseau, setListeReseau] = React.useState<any>([])

    const getDatas = async () => {
        // const res:any = await getData();
        const res:any = await getConstante('data')
        const pays:any = res[0].pays.replace(/\s+/g, '')
        if (pays in reseaux) {
          const reseauxPays = reseaux[pays as keyof typeof reseaux];
          setListeReseau(reseauxPays);
        } else {
            console.warn(`Le pays "${pays}" n'existe pas dans la constante reseaux.`);
        }
        setDatas(res[0])    
    }
    ...
    const showModal = useCallback(async () => {
      ...
      const valideContact = async (num:any) => {
        ...
        // ]
        const data = [
          {destinataire: num, pays: datas?.pays, reseau: datas?.reseau, expediteur: datas?.expediteur, reseauDestinataire: reseau}
        ]
        console.log(data)
        setCurrentNumero({ num: num.phoneNumbers[0].number, confNum : num.phoneNumbers[0].number})
      ...
      const handleNext = async () => {
        if(reseau == '')
          ...
          if(currentNumero.num != currentNumero.confNum)
          {
            alert('Les numéros ne sont pas identique')
            return
          }
        ...
        if(currentNumero){
          if(reseau != "WAVE")
          {
            switch (reseau)
            {
              case "ORANGE":
                ...
                if(!regex.test(currentNumero.num))
                {
                ...
                case "MOOV":
                  ...
                  if(!regex1.test(currentNumero.num))
                  {
                ...
                case "MTN":
                  ...
                  if(!regex2.test(currentNumero.num))
                  {
          ...
          const data = [
            {destinataire: {name: currentNumero.num, "phoneNumbers": [{number: currentNumero.num}]}, pays: datas?.pays, reseau: datas?.reseau, expediteur: datas?.expediteur, reseauDestinataire: reseau}
          ]
          await saveContante("data", JSON.stringify(data))

          navigation.navigate('TransactionInter')
        }else{
          ...
          <ScrollView contentContainerStyle={{flexGrow: 1}} style={{flex: 1}} showsHorizontalScrollIndicator={false}>
            ...
            <KeyboardAvoidingView behavior='padding' keyboardVerticalOffset={Platform.OS == 'ios' ? 20 : 0} style={{paddingLeft: 12, paddingRight: 12}}>
                ...
                <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 20, padding: 20, marginBottom: 30}}>
                    {listeReseau.map((row:any) => {
                      const isSelected = reseau === row.nom.toUpperCase();
                      // Définir la couleur de la bordure selon le réseau sélectionné
                      const borderColor = row.nom.toUpperCase().includes('WAVE')
                      ? '#1DC8FE'
                      : row.nom.toUpperCase().includes('WIZALL SN')
                      ? '#1DC8FE'
                      : row.nom.toUpperCase().includes('ORANGE')
                      ? '#FFB31B'
                      : row.nom.toUpperCase().includes('FREE')
                      ? '#010101'
                      : row.nom.toUpperCase().includes('MTN')
                      ? '#FFCC06'
                      : row.nom.toUpperCase().includes('MOOV')
                      ? '#005CA9'
                      : 'rgba(241, 245, 249, 1)';

                      return (
                        <TouchableOpacity key={row.id} style={{flexDirection: 'column',justifyContent:'center', alignItems: 'center', gap: 7}} onPress={() => setReseau(row.nom.toUpperCase())}>
                          <View style={{borderWidth: 2, width: 60, height: 60,justifyContent:'center', alignItems: 'center', borderRadius: 10, borderColor: isSelected ? borderColor : 'rgba(241, 245, 249, 1)'}}>
                              <Image source={row.logo} style={{ width: 48, height: 48, backgroundColor: 'cover', borderRadius: 10 }} />
                          </View>
                          <Text style={{fontWeight: 400, fontSize: 20}}>{row.nom}</Text>
                        </TouchableOpacity>
                      )
                    })}
                    {/* WAVE */}
                ...
                <Text style={{fontSize: 16, lineHeight: 24, marginTop: 30, marginLeft: 12, fontFamily: 'Bold'}}>Entrez son numéro de téléphone</Text>
                ...
                <View style={styles.inputBox}>
                    ...
                    <TextInput
                        ...
                        value={currentNumero.num}
                        onChangeText={(e) => setCurrentNumero({...currentNumero, num: e})}
                ...
                <Text style={{fontSize: 16, lineHeight: 24, marginTop: 30, marginLeft: 12, fontFamily: 'Bold'}}>Veuillez confirmer le numéro</Text>
                ...
                <View style={styles.inputBox}>
                    ...
                    <TextInput
                        ...
                        placeholder="00 00 00 00 00"
                        placeholderTextColor="#6b7280"
                        value={currentNumero.confNum}
                        onChangeText={(e) => setCurrentNumero({...currentNumero, confNum: e})}
                        keyboardType="phone-pad"
                        maxLength={10}
                    />
                </View>
...
export default ChoixNumeroDestinataireInter;
